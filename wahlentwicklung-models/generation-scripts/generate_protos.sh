...
PROTO_DIR="../../wahlentwicklung-models"
...
TS_OUTPUT_DIR="../../wahlentwicklung-ui/src/app/protos"

# Path to the protoc-gen-ts_proto.cmd (npm package)
PROTOC_GEN_TS_PATH="..\\..\\wahlentwicklung-ui\\node_modules\\.bin\\protoc-gen-ts.cmd"
...
START_TIME=$(date +%s%N)
protoc -I="$PROTO_DIR" --plugin=protoc-gen-ts_proto=$PROTOC_GEN_TS_PATH --ts_proto_out=$TS_OUTPUT_DIR "$PROTO_DIR/*.proto" --ts_proto_opt=esModuleInterop=true --ts_proto_opt=importSuffix=.js --ts_proto_opt=explicit_override=true
END_TIME=$(date +%s%N)
