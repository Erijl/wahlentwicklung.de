...
export class ElectionConstituenciesListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.year.set(y);
    this.svc.getElectionConstituencies(y).subscribe((rows) => {
      this.constituencies.set(rows);
      const promises = rows.map(async (c) => {
        const turnout = await this.fetchConstituencyTurnout(y, c.state_row_id, c.name);
        const shares = await this.fetchConstituencyShares(y, c.state_row_id, c.name);
        return { name: c.name, turnout, topParties: shares };
      });
  ...
  filtered = computed(() => {
    ...
    const arr = this.enriched().filter((c) => c.name.toLowerCase().includes(q));
    return arr.sort((a, b) => a.name.localeCompare(b.name));
  });

  private async fetchConstituencyTurnout(year: number, stateRowId: number, name: string): Promise<number> {
    try {
      const Database = require('better-sqlite3');
      const db = new Database('db/wahlentwicklung.db', { readonly: true, fileMustExist: true });
      const c = db.prepare('SELECT row_id FROM election_constituency WHERE election_year = ? AND state_id = ? AND name = ?').get(year, stateRowId, name) as { row_id: number } | undefined;
      if (!c) { db.close(); return 0; }
      const base = db.prepare('SELECT actualvoters_secondaryvote_definitive AS actual, eligiblevoters_secondaryvote_definitive AS eligible FROM constituency_vote_base WHERE election_year = ? AND state_id = ? AND constituency_id = ?').get(year, stateRowId, c.row_id) as { actual: number; eligible: number } | undefined;
      db.close();
  ...
  private async fetchConstituencyShares(year: number, stateRowId: number, name: string): Promise<Array<{ label: string; pct: number; color: string }>> {
    try {
      const Database = require('better-sqlite3');
      const db = new Database('db/wahlentwicklung.db', { readonly: true, fileMustExist: true });
      const c = db.prepare('SELECT row_id FROM election_constituency WHERE election_year = ? AND state_id = ? AND name = ?').get(year, stateRowId, name) as { row_id: number } | undefined;
      if (!c) { db.close(); return []; }
      const totalRow = db.prepare('SELECT SUM(secondaryvote_definitive) AS total FROM constituency_vote_party WHERE election_year = ? AND state_id = ? AND constituency_id = ?').get(year, stateRowId, c.row_id) as { total: number } | undefined;
      ...
      const rows = db.prepare(`
        ...
        ORDER BY pct DESC
      `).all(total, year, stateRowId, c.row_id) as Array<{ label: string; color: string; pct: number }>;
      db.close();
